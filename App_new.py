...
def get_chat_messages(id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, message, timestamp FROM messages WHERE recipient_id = ?", (id,)
    )
    messages = cursor.fetchall()
    conn.close()

    message_list = [
        {"id": message[0], "message": message[1], "time": message[2]}
...
def get_groups():
    ...
    username = data["username"] if "username" in data and data["username"] else None
